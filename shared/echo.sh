...
# Functions:
echo_pretty(){
 [[ "$1" == "-h" ]] && echo_info "decorated echo" && return
 local text=$1
...
echo_ok(){
 [[ "$1" == "-h" ]] && echo_info "Echo in green" && return
...
echo_info(){
  [[ "$1" == "-h" ]] && echo_info "Echo in blue" && return
...
echo_debug(){
  [[ "$1" == "-h" ]] && echo_info "Echo in white" && return
 [[ $CURRENT_LOG_LVL -le $LOG_LVL_DEBUG ]] && echo "$@"
}

echo_warn(){
 [[ "$1" == "-h" ]] && echo_info "Echo in yellow" && return
...
echo_err(){
 [[ "$1" == "-h" ]] && echo_info "Echo in red" && return
