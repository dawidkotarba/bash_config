...
source $SHELL_SHARED_PATH/echo.sh
source $SHELL_SHARED_PATH/checks.sh
...
_source_modules_aliases(){
 [[ "$1" == "-h" ]] && show_help $funcstack[1] && return
 for file in $(find $SHELL_MODULES_PATH -type f -name help.sh); do _source_if_exists "$file"; done
 for file in $(find $SHELL_MODULES_PATH -type f -name aliases.sh); do _source_if_exists "$file"; done
}
...
_source_if_exists $SHELL_APPS_PATH/z/z.sh
